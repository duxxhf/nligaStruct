%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%
%
%  An example for structural analysis of the benchmark problem -pinched cylinder using RM shell
%
%  ---------------------------------------
%
...
if strcmp(meshType, 'T-SPLINE')
    R = 300; L = 600;
    filename = 'geoOctantCylinderR3.iga';
    geo  = readIgaFile(filename); 
    mesh = buildIgaMesh( geo );
elseif strcmp(meshType, 'NURBS')
    R = 300; L = 600; p = 3; q = p; elemX = 5; elemY = elemX; 
    geo  = geoOctantCylindricalShellNurbs(R,L,p,q,elemX,elemY);
    mesh = buildIgaMesh( geo );
end
...
rightNodes    = find( abs(mesh.coords(:,3)) < tol );
topNodes      = find( abs(mesh.coords(:,2)-L/2) < tol );
leftNodes     = find( abs(mesh.coords(:,1)) < tol );
% sort leftNodes
leftNodes = sortrows([floor(mesh.coords(leftNodes,2)*1000)/1000, leftNodes],1,'ascend');
leftNodes = leftNodes(:,2);
% find forced nodes
forceNode = leftNodes(end);
...
% rigid diaphragm, ux = uz = theta_y = 0
...
% right symmetric boundary, uz = theta_x = theta_y = 0
dbc = [dbc; rightNodes,      3*ones(size(rightNodes)),    zeros(size(rightNodes))];     
dbc = [dbc; rightNodes,      4*ones(size(rightNodes)),    zeros(size(rightNodes))];
...
fprintf('The vertical displacement at the forced node is %12.6e.\n',-u(forceNode*6-3));
