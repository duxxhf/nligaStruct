%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%
%
%  An example of free form cylinder for structural vibration using RM shell
%
%  ---------------------------------------
%
...
%****************************************************************%
bc = 3;  % =1 both ends clamped; = 2 one end clamped, another free
...
% fixedNodes  = unique([bottomNodes;topNodes]);
if bc == 1  % both ends clamped
    fixedNodes  = unique([bottomNodes;topNodes]);
    fixed_dof   = zeros(length(fixedNodes)*6,1);
    for i = 1:6
        fixed_dof(i:6:end) = (fixedNodes-1)*6+i;  % ux
    end
elseif bc == 2  % one end clamped, another free
    fixedNodes  = unique(bottomNodes);
    fixed_dof   = zeros(length(fixedNodes)*6,1);
    for i = 1:6
        fixed_dof(i:6:end) = (fixedNodes-1)*6+i;  % ux
    end
elseif bc == 3  % one end clamped, another simply supported uy = 0
    fixedNodes1 = unique(bottomNodes);
    fixedNodes2 = unique(topNodes);
    fixed_dof1  = zeros(length(fixedNodes1)*6,1);
    for i = 1:6
        fixed_dof1(i:6:end) = (fixedNodes1-1)*6+i;  % ux
    end
    fixed_dof2  = (fixedNodes2-1)*6+2;  % uy
