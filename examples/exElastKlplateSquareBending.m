...
elseif strcmp(meshType, 'NURBS')
    pts = [0,0]; p = 3; q = p; elemX = 10; elemY = elemX; 
    geo  = geoRectangularPlate( a,b,pts,p,q,elemX,elemY );
    mesh = buildIgaMesh( geo );
end

toc
bc   = 2; % boundary conditions; =1 clamped; =2 simply supported
...
bc_nodes = (unique([bottomNodes;nextBottomNodes;topNodes;nextTopNodes; ...
...
figure(2);
suptitle( 'Static Bending of a Square Plate Using KL based IGA') ;
for gg = 1:4
    subplot(2,2,gg);
